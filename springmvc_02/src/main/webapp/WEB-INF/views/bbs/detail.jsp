		...
		<c:forEach items="${clist }" var="k">
			<div style="border: 1px solid #cc00cc; width: 500px; margin: 5px; padding: 5px;">
				<form action="/comment_delete" method="post">
					...
					<p> 날짜: <pre>${k.write_date }</pre></p>
					<%-- 실제는 로그인 성공 후 관리자나 글쓴 본인인 경우만 삭제 가능 --%>
					<input type="submit" value="댓글 삭제">
					<%-- 페이지 안 바뀌면 ajax 사용 / 페이지 바뀌면 mvc 사용 --%>
					<%-- 컨트롤러가서 DB 삭제 후 다시 이곳으로 와야한다.(나중에 ajax로 변경하자) --%>
